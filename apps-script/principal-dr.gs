...
// Calendar id) -- confirmed 2026-09-02 for LMS1-4. LMS5/LMS6 added
// 2026-09-07: their calendars now exist and are accessible (Priyanka
// Sharma's has real, current events; Nikita Sen's exists but is still
// empty) -- verified via a separate Calendar tool, not yet re-confirmed
// from THIS Apps Script project's own execution identity. Run
// testCalendarAccess() after deploying to get that direct confirmation;
// if either somehow isn't reachable from here, pdrReadSupportSessionsForDate_
// already fails safe (getCalendarById returns null -> [] -> 0), so this
// is safe to ship either way. Support Sessions are logged here, tagged
// "SS" per the tag convention (MA/DISP/SS/NBK/MEET).
const PDR_PRINCIPAL_PERSONAL_CALENDARS = {
  ...
  LMS5: 'priyanka.sharma@lms.org.in',
  LMS6: 'nikita.sen@lms.org.in',
};
...
function testCalendarAccess() {
  Object.values(PDR_PRINCIPAL_PERSONAL_CALENDARS).forEach(function (email) {
